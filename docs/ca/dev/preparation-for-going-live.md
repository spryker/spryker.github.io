...
- Please make sure that you have implemented (where applicable) Jenkins [stability and performance improvements](https://docs.spryker.com/docs/scos/dev/tutorials-and-howtos/howtos/howto-reduce-jenkins-execution-costs-without-refactoring.html) and are observing general [stability best practises](https://docs.spryker.com/docs/ca/dev/best-practices/best-practises-jenkins-stability.html) for Publish and Sync with Jenkins.
...
- *Validate BI integration and data analytics connections* These should under no circumstances be connected to the production database, but the provided read replica that works with the same credentials as the production database. Nothing apart from your application should now be connected to your production database and write and read from it. 
